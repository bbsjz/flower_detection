...
let list=[0,0,0]
getLabel("search/getFlowerById", msg).then((response) => {
    if (response.status === 200) {
        ...
        list[0]="http://localhost:8080/"+response.data.url1.substring(7);
        list[1]="http://localhost:8080/"+response.data.url2.substring(7);
        list[2]="http://localhost:8080/"+response.data.url3.substring(7);
        plant.value.pics = list;
        load.value = true;
    }
    else {
