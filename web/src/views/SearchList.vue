...
getLabelFromText('search/findByName', key_words).then((value) => {
    if (value.status == 200) {
        result_list.value = value.data
        for(var i=0;i<result_list.value.length;i++)
        {
            result_list.value[i].url="http://localhost:8080/"+result_list.value[i].url.substring(8)
        }
